...
const getTodo = (id) => axios.get(`${BASE_URL}/todo/${id}`);
...
function* getList(): any {
    ...
    yield put({ type: GET_TODO, payload:{response} });
...
function* todoSaga() {
  yield all([takeLatest(FETCH_TODO_REQUEST, fetchTodoSaga),takeLatest(ADD_TODO, addList),
    takeLatest(DELETE_TODO, deleteList),
    takeLatest(GET_TODO, getList)]);
}
